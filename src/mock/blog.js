...
Mock.mock("/api/blogtype", "get",{ // 要拦截的地址 要拦截的Ajax请求的类型 数据模板
...
Mock.mock(/^\/api\/blog(\?.+)?$/, "get", function(options){ // 函数的返回结果作为模板
   const query = qs.parse(options.url); //querystring.pares() 格式化字符串
   const data = {
    "total|2000-3000": 0,
    [`rows|${query.limit||10}`]:[{
        id:"@guid", // 独特的id
        title:"@ctitle(1,50)", // 生成中文标题
        description:"@cparagraph(1, 10)", //生成1-10个句子
        category:{
            "id|1-10": 0,
            name:"分类@id",

        },
        "scanNumber|0-3000": 0, // 浏览次数
        "commentNumber|0-300": 30,
        "thumb|1":[
          // Mock.Random.image("300x250","#000", "#fff","Random Image"),
          "@image(300x250, @color, #fff, @natural)", // 模板字符串，动态调用这个函数
          // @是调用函数，随机生成一个标题
          null
        ], // 随机生成一张图片,数组里可能有缩略图也可能没有
